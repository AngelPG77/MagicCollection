...
import com.pga.magiccollection.domain.usecase.wantlist.AddCardToWantListUseCase
import com.pga.magiccollection.domain.usecase.wantlist.GetWantListByIdUseCase
...
data class WantListAddCardUiState(
    val query: String = "",
...
class WantListAddCardViewModel @Inject constructor(
    ...
    private val wantListLocalId: Long = savedStateHandle.get<Long>("localId") ?: 0L

    private val _uiState = MutableStateFlow(WantListAddCardUiState())
    val uiState: StateFlow<WantListAddCardUiState> = _uiState.asStateFlow()
    ...
    private fun observeIndexStatus() {
        viewModelScope.launch {
            bootstrapCardIndexUseCase.hasIndexData().collect { hasData ->
                android.util.Log.d("WantListAddCardVM", "Index has data: $hasData")
            ...
            getAppPreferencesUseCase()
                ...
                .collect { (lang, availableLangs) ->
                    ...
                    try {
                        ...
                        if (!hasLocalData) {
                            android.util.Log.d("WantListAddCardVM", "Proactively bootstrapping index for $lang")
                            bootstrapCardIndexUseCase(lang)
                        }
                    } catch (e: Exception) {
                        android.util.Log.e("WantListAddCardVM", "Error checking/bootstrapping index", e)
                    }
                }
        }
    }
        ...
        viewModelScope.launch {
            ...
            .flatMapLatest { indexQuery ->
                if (indexQuery == null) {
                    ...
                    android.util.Log.d("WantListAddCardVM", "Performing local search: ${indexQuery.searchText}")
                    observeIndexedCardsUseCase(indexQuery)
                }
            }
            .collect { cards ->
                android.util.Log.d("WantListAddCardVM", "Found ${cards.size} local results")
            ...
            try {
                ...
                _uiState.update { 
                    it.copy(
                        ...
                        message = "wantlist_card_added_success",
                        isSearchPerformed = false
                    ) 
                }
            } catch (e: Exception) {
